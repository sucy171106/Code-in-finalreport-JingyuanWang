...
for i in range(times):
    ...
    # restore everytime
    testerrs_all = testerrs_all+np.array(testerrs)
    trainerrs_all=trainerrs_all+np.array(trainerrs)
    n_supports_all = n_supports_all+np.array(n_supports)
    ws_all=ws_all+np.array(ws)

# compute the mean
testerrs_all=testerrs_all/times
trainerrs_all=trainerrs_all/times
n_supports_all=n_supports_all/times
ws_all=ws_all/times
...
with open('q7testerrs.csv', 'w', encoding='utf-8', newline='') as file_obj:
    # create the object
...
with open('q7n_supports.csv', 'w', encoding='utf-8', newline='') as file_obj:
    # create the object
    writer = csv.writer(file_obj)
    writer.writerow((n_supports))

# export the train error in CSV file
with open('q7trainerrs.csv', 'w', encoding='utf-8', newline='') as file_obj:
    # create the object
    writer = csv.writer(file_obj)
    writer.writerow((trainerrs))

# export the norm of the coefficients in CSV file
with open('q7ws.csv', 'w', encoding='utf-8', newline='') as file_obj:
    # create the object
    writer = csv.writer(file_obj)
    writer.writerow((ws))